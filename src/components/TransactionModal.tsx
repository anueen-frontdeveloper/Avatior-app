...
type Method = { id: number; name: string; icon: string; };

type DepositModalProps = {
  visible: boolean;
  onClose: () => void;
  onSelectMethod: (method: Method) => void; // <-- The essential new prop
};

const DepositModal: React.FC<DepositModalProps> = ({ visible, onClose, onSelectMethod }) => {
          ...
          <ScrollView contentContainerStyle={styles.optionsWrapper}>
            {paymentMethods.map((item) => (
              <TouchableOpacity
                ...
                onPress={() => onSelectMethod(item)} // <-- Call the function on press
              >
